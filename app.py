...
app.config['MYSQL_DATABASE_DB'] = 'cs460'
...
def getUsersPhotos(uid):
    conn.cursor()
    cursor.execute(
        "SELECT P.imgdata, P.photo_id, P.caption, P.num_likes FROM Photos P WHERE P.user_id = '{0}'".format(uid))
...
def getUsersAlbums(uid):
    conn.cursor()
    cursor.execute(
        "SELECT A.albums_id, A.name FROM ALbums A WHERE A.user_id = '{0}'".format(uid))
    test = cursor.fetchall()  # NOTE list of tuples, [(imgdata, pid), ...]
    return test
...
def getPhotosFromAlbumId(album_id):
    conn.cursor()
    cursor.execute(
        "SELECT P.imgdata, P.photo_id, P.caption, P.num_likes FROM Photos P WHERE P.albums_id = '{0}'".format(album_id))
    test = cursor.fetchall()  # NOTE list of tuples, [(imgdata, pid), ...]
    return test
...
def getAlbumNameFromAlbumId(album_id):
    conn.cursor()
    cursor.execute(
        "SELECT A.name FROM Albums A WHERE A.albums_id = '{0}'".format(album_id))
...
def getAllTagNames():
    conn.cursor()
    cursor.execute(
        "SELECT T.name, T.tag_id FROM Tags T ORDER BY T.num_photos DESC")
    test = cursor.fetchall()  # NOTE list of tuples, [(imgdata, pid), ...]
    return test
...
def getTagIdFromTagName(tag_name):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT T.tag_id FROM Tags T WHERE T.name = '{0}'".format(tag_name))
...
def getTagNameFromTagId(tag_id):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT T.name FROM Tags T WHERE T.tag_id = '{0}'".format(tag_id))
    return cursor.fetchall()[0][0]
...
def getUserNameFromUserId(user_id):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT U.first_name, U.last_name FROM Users U WHERE U.user_id = '{0}'".format(user_id))
...
def getEmailFromUserId(user_id):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT U.email FROM Users U WHERE U.user_id = '{0}'".format(user_id))
...
def getLikeUsersFromPhotoId(photo_id):
    conn.cursor()
    cursor.execute(
        "SELECT L.user_id FROM Likes L WHERE L.photo_id = '{0}'".format(photo_id))
    users = cursor.fetchall()
    user_names = []
    for user in users:
...
def getAllPhotoLikes():
    ...
    for photo_id in photo_ids:
        photo_id = photo_id[0]
        cursor.execute(
            "SELECT COUNT(*) FROM Likes L WHERE (L.photo_id = '{0}')".format(photo_id))
...
def increaseScore(uid):
    cursor.execute(
        "SELECT U.score FROM Users U WHERE U.user_id = '{0}'".format(uid))
    user_score = cursor.fetchall()[0][0]
    cursor.execute("UPDATE Users SET score = '{0}' WHERE user_id = '{1}'".format(
        user_score + 1, uid))
    conn.commit()
    return
...
def getTopScoreUsers():
    conn.cursor()
    cursor.execute(
        "SELECT U.score, U.first_name, U.last_name FROM Users U WHERE NOT U.user_id = -1 ORDER BY U.score DESC")
...
def getPhotosFromTagId(tag_id, user_id=None):
    ...
    if user_id == None:
        cursor.execute(
            "SELECT P.imgdata, P.photo_id, P.caption, P.num_likes FROM Tagged T, Photos P WHERE (P.photo_id = T.photo_id AND T.tag_id = '{0}')".format(tag_id))
        # NOTE list of tuples, [(imgdata, pid), ...]
...
def getUserIdFromPhotoId(photo_id):
    conn.cursor()
    cursor.execute(
        "SELECT P.user_id FROM Photos P WHERE P.photo_id = '{0}'".format(photo_id))
    test = cursor.fetchall()[0][0]
    return test
...
def create_album(album_name):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT A.name FROM Albums A WHERE (A.name = '{0}' AND A.user_id = '{1}')".format(
        album_name, user_id))
    alb_name = cursor.fetchall()
    if not alb_name:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Albums (user_id, name) VALUES ('{0}', '{1}')".format(
            user_id, album_name))
        conn.commit()
    cursor.execute("SELECT A.albums_id FROM Albums A WHERE (A.name = '{0}' AND A.user_id = '{1}')".format(
        album_name, user_id))
...
def create_tag(tag_name):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT T.name FROM Tags T WHERE T.name = '{0}'".format(tag_name))
    t_name = cursor.fetchall()
    if not t_name:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO Tags (name, num_photos) VALUES ('{0}', '{1}')".format(tag_name, 0))
        conn.commit()
    cursor.execute(
        "SELECT T.tag_id FROM Tags T WHERE T.name = '{0}'".format(tag_name))
...
def delete_file():
    if request.method == 'POST':
        ...
        conn.cursor()
        cursor.execute(
            "DELETE FROM Photos P WHERE P.photo_id = '{0}'".format(photo_id))
        conn.commit()
...
def delete_album():
    if request.method == 'POST':
        ...
        conn.cursor()
        cursor.execute(
            "DELETE FROM Albums A WHERE A.albums_id = '{0}'".format(album_id))
        conn.commit()
...
def friends():
    ...
    conn.cursor()
    cursor.execute(
        "SELECT F.user_id2 FROM Friends F WHERE F.user_id1 = '{0}'".format(str(user_id)))
    friendid_query = cursor.fetchall()
    friend_ids = [friend[0] for friend in friendid_query]
    friend_query = []
    for id in friend_ids:
        cursor.execute(
            "SELECT U.email FROM Users U WHERE U.user_id = '{0}'".format(id))
...
def friend_rec():
    try:
        ...
        conn.cursor()
        cursor.execute(
            "SELECT F.user_id2 FROM Friends F WHERE F.user_id1 = '{0}'".format(uid))
...
def add_tag():
    ...
    try:
        conn.cursor()
        cursor.execute("INSERT INTO Tagged (photo_id, tag_id, user_id) VALUES ('{0}', '{1}', '{2}')".format(
            photo_id, tag_id, uid))
        conn.commit()
        cursor.execute(
            "SELECT T.num_photos FROM Tags T WHERE T.name = '{0}'".format(tag_name))
        t_photoVal = cursor.fetchall()[0][0]
        cursor.execute("UPDATE Tags SET num_photos = '{0}' WHERE name = '{1}'".format(
            t_photoVal + 1, tag_name))
        conn.commit()
...
def leave_comment():
    ...
    try:
        conn.cursor()
        cursor.execute("INSERT INTO Comments (user_id, photo_id, text) VALUES ('{0}', '{1}', '{2}')".format(
            uid, photo_id, comment))
        conn.commit()
        increaseScore(uid)
...
def like_photo():
    ...
    try:
        conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM Likes L WHERE (L.photo_id = '{0}' AND L.user_id = '{1}')".format(photo_id, uid))
        if (cursor.fetchall()[0][0] == 0):
            cursor.execute(
                "INSERT INTO Likes (user_id, photo_id) VALUES ('{0}', '{1}')".format(uid, photo_id))
            conn.commit()
            cursor.execute(
                "SELECT P.num_likes FROM Photos P WHERE P.photo_id = '{0}'".format(photo_id))
            t_likeVal = cursor.fetchall()[0][0]
            cursor.execute("UPDATE Photos SET num_likes = '{0}' WHERE photo_id = '{1}'".format(
                t_likeVal + 1, photo_id))
            conn.commit()
...
def search_comments():
    ...
    conn.cursor()
    cursor.execute(
        "SELECT C.user_id FROM Comments C WHERE (C.text = '{0}' AND NOT C.user_id = -1)".format(search_comment))
    ...
    for user_id in user_ids:
        user_id = user_id[0]
        cursor.execute(
            "SELECT COUNT(*) FROM Comments C WHERE C.user_id = '{0}'".format(user_id))
